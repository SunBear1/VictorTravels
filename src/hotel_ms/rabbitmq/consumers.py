...
from service.hotels_for_trip import get_offers_for_hotel, get_hotel_for_offer, update_left_rooms_in_hotel, \
    check_if_hotel_booked_up
...
def consume_eventhub_ms_event(ch, method, properties, body):
    ...
    logger.info(f"Offers for {hotel_id} are {offers}.")
    ...
    logger.info(msg=f"Other trip offers with the same hotel ID: {offers}")
