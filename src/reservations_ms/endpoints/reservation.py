...
async def make_reservation(trip_id: str):
    ...
    try:
        ...
        RabbitMQClient.send_data_to_queue(queue_name=PURCHASES_QUEUE_NAME, exchange_name=PURCHASES_EXCHANGE_NAME,
        ...
        RabbitMQClient.send_data_to_queue(queue_name=RESERVATIONS_QUEUE_NAME, exchange_name=RESERVATIONS_EXCHANGE_NAME,
                                          payload=msg_to_director_ms)

        # tmp = MongoDBClient.trips_collection.update_one(filter={"_id": xd.inserted_id}, update={"$set": {"reserved": True}})
