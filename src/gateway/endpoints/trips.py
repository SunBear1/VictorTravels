...
async def get_trips(
    ...
    try:
        query_params = {
            ...
            "dateFrom": date_from,
            "dateTo": date_to,
            "departureRegion": departure_region,
            "arrivalRegion": arrival_region,
            "transport": transport,
            "order": order,
            "diet": diet,
            "maxPrice": max_price
...
async def get_trip_price(
        adults: Optional[int] = Query(None, ge=1),
        kids_to_3yo: Optional[int] = Query(None, ge=0),
        kids_to_10yo: Optional[int] = Query(None, ge=0),
        kids_to_18yo: Optional[int] = Query(None, ge=0),
        number_of_days: Optional[int] = Query(None, ge=1),
        room_cost: Optional[int] = Query(None, ge=0),
        diet_cost: Optional[int] = Query(None, ge=0),
        transport_to_cost: Optional[int] = Query(None, ge=0),
        transport_from_cost: Optional[int] = Query(None, ge=0),
):
    """
    Return information about price of a specific trip details
    """
    try:
        query_params = {
            "adults": adults,
            ...
            "roomCost": room_cost,
            "dietCost": diet_cost,
            "transportToCost": transport_to_cost,
